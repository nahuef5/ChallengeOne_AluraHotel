...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		valorPanel.setLayout(new FlowLayout(FlowLayout.CENTER));
		valorPanel.setBackground(SystemColor.text); 
		valorPanel.setBounds(68, 328, 260, 33);
		...
		btnsiguiente.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if((txtFechaE.getDate() != null && txtFechaS.getDate() != null)){
					...
					LocalDate fechaMaxima= today.plusYears(5);
					if((fechaEntrada.isEqual(today) || fechaEntrada.isAfter(today)) && fechaEntrada.isBefore(fechaMaxima)
							&& fechaSalida.isAfter(fechaEntrada) && fechaSalida.isBefore(fechaEntrada.plusMonths(1))
							) {
						guardarReserva();
					}
					else{
						JOptionPane.showMessageDialog(null,
		                        "La fecha de entrada debe ser igual a la de hoy o posterior, con un maximo de 5 años.\n"
		                        + "La fecha de salida debe ser posterior a la de entrada y maximo un mes despues.");
					}
				}
	...
	private void guardarReserva(){
		...
		try(Connection connection = new ConnectionFactory().connection()){
        	...
        	var entrada = ((JTextField) txtFechaE.getDateEditor().getUiComponent()).getText();
			var salida = ((JTextField) txtFechaS.getDateEditor().getUiComponent()).getText();
        	...
        	var formaPago=txtFormaPago.getSelectedItem().toString();
        	...
        	int id=reservaController.getLastIdReserva();
        	RegistroHuesped registroHuesped = new RegistroHuesped(id);
	...
	private LocalDate getDateFromField(JDateChooser date) {
	...
	private BigDecimal calculate(LocalDate fechaEntrada, LocalDate fechaSalida) {
		return Calculadora.valueOfDays(fechaEntrada, fechaSalida);
	}
	private void showValue() {
		LocalDate fechaEntrada=getDateFromField(txtFechaE);
		LocalDate fechaSalida=getDateFromField(txtFechaS);
		if(fechaEntrada!=null && fechaSalida!=null) {
			var value=calculate(fechaEntrada, fechaSalida);
